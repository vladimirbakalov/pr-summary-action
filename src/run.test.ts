...
describe("run", () => {
  ...
  it("generates a summary and creates a comment on first run", async () => {
    ...
    const postedBody = createComment.mock.calls[0][0].body as string;
    expect(postedBody).toContain("## What changed\n- Added a.ts");
    expect(postedBody).toContain("— generated by [BYOK AI PR Summary](https://github.com/vladimirbakalov/pr-summary-action)");
  });
